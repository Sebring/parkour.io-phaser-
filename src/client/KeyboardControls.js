	...
	constructor(clientEngine) {
		...
		this.gameEngine.on('client__preStep', () => {
			for (let keyName of Object.keys(this.boundKeys)) {
				let key = this.keyState[keyName]
				if (key && !key.handled) {
						this.clientEngine.sendInput(this.boundKeys[keyName].actionName, key)
						key.handled = true
				}
			}
			this.keyStates = []
	...
	onKeyChange(e, isDown) {
		...
		if (keyName && this.boundKeys[keyName]) {
			...
			if (this.keyState[keyName] == null) {
				...
				if (this.keyState[keyName].keyDown !== isDown) {
					this.keyState[keyName].handled = false
				}
			}

			this.keyState[keyName].keyDown = isDown
			this.keyState[keyName].keyUp = !isDown
