...
import HSHGCollisionDetection from './HSHGCollisionDetection'
import BruteCollisionDetection from './BruteCollisionDetection'
 ...
 */
class PkPhysicsEngine extends PhysicsEngine {
	...
	*/
	constructor(options) {
		...
		if (options.collisions && options.collisions.type === 'HSHG') {
			...
			this.collisionDetection = new BruteCollisionDetection(options.collisions);
		}
...
/* 
	...
	step(dt, objectFilter) {
		...
		let activeRooms = this.gameEngine.getAllActiveRooms()
		if (!activeRooms)
			return
		// in each room, step through objects
		for (let room of activeRooms) {
			let objects = this.gameEngine.getAllObjectsInRoom(room)
			...
			this.collisionDetection.detect(Object.keys(objects), room)
		}
	}
...
/* 		// each object should advance
		...
		// detect collisions
		this.collisionDetection.detect(Object.keys(objects), '/lobby')
	} */
}

export default PkPhysicsEngine;
